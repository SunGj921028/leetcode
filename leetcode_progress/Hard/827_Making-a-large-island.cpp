...
private:
    int exploreIsland(vector<vector<int>>& grid, int islandId, int currentRow,
        ...
        grid[currentRow][currentColumn] = islandId; // Mark the cell as visited
        return 1 +
...
public:
    int largestIsland(vector<vector<int>>& grid) {
        ...
        for (int currentRow = 0; currentRow < grid.size(); ++currentRow) {
            for (int currentColumn = 0; currentColumn < grid[0].size(); ++currentColumn) {
            ...
            for (int currentColumn = 0; currentColumn < grid[0].size(); ++currentColumn) {
                if (grid[currentRow][currentColumn] == 0) {
                    ...
                    if (currentColumn - 1 >= 0 &&
                        grid[currentRow][currentColumn - 1] > 1) {
                        neighboringIslands.insert(
                            grid[currentRow][currentColumn - 1]); // The current grid store the island id
                    }
